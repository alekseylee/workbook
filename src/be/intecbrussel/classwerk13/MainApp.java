package be.intecbrussel.classwerk13;

public class MainApp {
    public static void main(String[] args) {

        Panthera panthera = new Panthera('m','m', "black", 200);
        Cheetah cheetah = new Cheetah('m','m',"yellow with black spots", 100);
        NorwegianForestCat norwegianForestCat = new NorwegianForestCat('s','s',"grey", 15, "Kitty", 5, 123, 555);
        AndeanMountainCat andeanMountainCat = new AndeanMountainCat('s','s',"yellow with black lines", 14, "Andy", 9, 124, 554);
        SiameseCat siameseCat = new SiameseCat('s','s', "white", 5, "Simba", 4, 333,543);
        TurkishVanCat turkishVanCat = new TurkishVanCat('s','s', "white", 3, "Snowball", 8, 390, 543);
        PersianCat persianCat = new PersianCat('s','s',"ginger", 11, "Polly", 14, 333,543);

        System.out.println(panthera);
        panthera.hunt();
        panthera.roar();

        System.out.println(cheetah);
        cheetah.run(5.5);
        cheetah.kiauuww();

        System.out.println(norwegianForestCat);
        norwegianForestCat.miauwInNorvegian();

        System.out.println(andeanMountainCat);
        andeanMountainCat.miauw();
        andeanMountainCat.isAlive();

        System.out.println(siameseCat);

        System.out.println(persianCat);
        persianCat.miauw();

        System.out.println(turkishVanCat);
        System.out.println(turkishVanCat.nickname);


//        Felis felis01 = new Felis();
//        Felis felis02 = new Felis();
//        Felis felis03 = new Felis();
//        Felis felis04 = new Felis();
//        Felis felis05 = new Felis();
//        Felis felis06 = new Felis();
//        Felis felis07 = new Felis();
//        Felis[] array = {felis01, felis02, felis03, felis04, felis05, felis06, felis07};
//        for (Felis s : array) {
//            System.out.println(s);
//        }



//        System.out.println(persianCat);
//        System.out.println(siameseCat);
//        System.out.println(persianCat.equals(siameseCat));





//        System.out.println(Animal.animalCount);
//        System.out.println(Felis.getVaccinatedCats());



    }
}
